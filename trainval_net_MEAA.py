...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            rois_label,out_d_inst, out_d1, out_d2, out_d3, \
            feat1_p, feat2_p, feat3_p = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
            # cam_out_d1, cam_out_d2, cam_out_d3 

            loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
                   + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() 
                   # + cls_prob_va  # minimize variance, maximize entropy
            ...
            # source domain label
            #---
            # domain_s1 = Variable(torch.zeros(cam_out_d1.size(0)).long().cuda())
            # domain_s1_cam = Variable(torch.zeros(cam_out_d2.size(0)).long().cuda())
            # domain_s3_cam = Variable(torch.zeros(cam_out_d3.size(0)).long().cuda())


            # source_feat2_p = source_feat3_p = Variable(torch.zeros(feat2_p.size(0))).long().cuda()
            # source_feat2_p = source_feat3_p = Variable(torch.zeros(feat2_p.size(1))).long().cuda()
            
            #---

            domain_s2 = domain_s3 = Variable(torch.zeros(out_d2.size(0)).long().cuda())
            # print('feat2_p: ', feat2_p.shape) #([1, 2])

            # print('out_d2: ', out_d2.shape) #([1, 2])

            # print('source_feat2_p: ', source_feat2_p.shape) # [1]
            # print('domain_s2: ', domain_s2.shape) # [1]
            ...
            # new losses
            feat1_s_p = 0.5 * torch.mean(feat1_p ** 2)
            feat2_s_p = 0.5 * torch.mean(feat2_p ** 2)
            feat3_s_p = 0.5 * torch.mean(feat3_p ** 2)

            # print('\nfeat2_p: ', feat2_p.shape, feat2_p)
            # print('\nsource+feat2+p: ', source_feat2_p.shape)
            # feat2_s_p = 0.5 * CrossEntropy(feat2_p.squeeze(), source_feat2_p) * 0.2
            # feat3_s_p = 0.5 * CrossEntropy(feat3_p.squeeze(), source_feat3_p)

            # dloss_s12 = 0.5 * torch.mean(out_d12 ** 2)
            # dloss_s13 = 0.5 * torch.mean(out_d13 ** 2)
            # dloss_s1_cam = 0.5 * CrossEntropy(cam_out_d1, domain_s1) * 0.15
            # dloss_s1_cam2 = 0.5 * CrossEntropy(cam_out_d2, domain_s1_cam) * 0.15
            # dloss_s1_cam3 = 0.5 * CrossEntropy(cam_out_d3, domain_s3_cam) * 0.15
            ...
            out_d_inst, out_d1, out_d2, out_d3, \
            feat1_p, feat2_p, feat3_p = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, target=True)
            
            # cam_out_d1, cam_out_d2, cam_out_d3 = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, target=True)

            # target domain label
            #---
            # domain_t1 = Variable(torch.ones(cam_out_d1.size(0)).long().cuda())
            # domain_t1_cam = Variable(torch.ones(cam_out_d2.size(0)).long().cuda())
            # domain_t3_cam = Variable(torch.ones(cam_out_d3.size(0)).long().cuda())

            # target_feat2_p = target_feat3_p = Variable(torch.zeros(feat2_p.size(0))).long().cuda()
            ...
            # new losses
            feat1_t_p = 0.5 * torch.mean((1 - feat1_p) ** 2)
            feat2_t_p = 0.5 * torch.mean((1 - feat2_p) ** 2)
            feat3_t_p = 0.5 * torch.mean((1 - feat3_p) ** 2)

            # feat2_t_p = 0.5 * CrossEntropy(feat2_p, target_feat2_p) * 0.2
            # feat3_t_p = 0.5 * CrossEntropy(feat3_p, target_feat3_p)

            # dloss_t12 = 0.5 * torch.mean((1 - out_d12) ** 2)
            # dloss_t13 = 0.5 * torch.mean((1 - out_d13) ** 2)
            # dloss_t1_cam = 0.5 * CrossEntropy(cam_out_d1, domain_t1) * 0.15
            # dloss_t1_cam2 = 0.5 * CrossEntropy(cam_out_d2, domain_t1_cam) * 0.15
            # dloss_t1_cam3 = 0.5 * CrossEntropy(cam_out_d3, domain_t3_cam) * 0.15

            if args.dataset == 'sim10k':
             
                loss += (dloss_s1 + dloss_t1 + dloss_s2 + \
                    dloss_t2 + dloss_s3 + dloss_t3 + dloss_s_p + dloss_t_p + \
                    feat1_s_p + feat2_s_p + feat3_s_p + \
                    feat1_t_p + feat2_t_p + feat3_t_p) * args.eta 
                ...
                loss += (dloss_s1 + dloss_t1 + dloss_s2 + \
                    dloss_t2 + dloss_s3 + dloss_t3 + dloss_s_p + dloss_t_p + \
                    feat1_s_p + feat2_s_p + feat3_s_p + \
                    feat1_t_p + feat2_t_p + feat3_t_p) 

                # remove D2, D3
                # loss += (dloss_s1 + dloss_t1 + dloss_s_p + dloss_t_p + \
                #     feat1_s_p + feat2_s_p + feat3_s_p + \
                #     feat1_t_p + feat2_t_p + feat3_t_p) 
            ...
            if step % args.disp_interval == 0:
                ...
                # logger.write(
                  ...
                  #      args.eta))
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, dloss s1: %.4f, dloss t1: %.4f, dloss s2: %.4f, dloss t2: %.4f, dloss s3: %.4f, dloss t3: %.4f, dinst_s: %.4f, dinst_t: %.4f, \
                    eta: %.4f"\
                    % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, dloss_s1, dloss_t1, dloss_s2, dloss_t2, dloss_s3, dloss_t3, dloss_s_p, dloss_t_p,
                       args.eta))
                # logger.close()
